...
export const BattleCard: React.FC<BattleCardProps> = ({
  ...
  const canSettle = !battle.settled && battle.endTime <= BigInt(Math.floor(Date.now() / 1000));
  ...
  const progressValue = calculateProgressValue(battle.token1Votes, battle.token2Votes);
                  ...
                  <TooltipTrigger>
                    <div className="text-sm">
                      ...
                      <div className="flex items-center gap-1 text-muted-foreground">
                        ...
                        <span>{battle.token1Votes.toString()}</span>
                      </div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>Supply: {token1 ? formatTokenAmount(token1.supply) : 'N/A'}</p>
            ...
            <div className="col-span-1 flex items-center justify-center">
              <div className="w-full space-y-2">
                ...
                <p className="text-xs text-center text-muted-foreground">
                  {progressValue.toFixed(1)}% - {(100 - progressValue).toFixed(1)}%
                </p>
              </div>
            </div>
                  ...
                  <TooltipTrigger>
                    <div className="text-sm text-right">
                      ...
                      <div className="flex items-center justify-end gap-1 text-muted-foreground">
                        <Users className="w-3 h-3" />
                        <span>{battle.token2Votes.toString()}</span>
                      </div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>Supply: {token2 ? formatTokenAmount(token2.supply) : 'N/A'}</p>
                        ...
                        >
                          {isVoting === battle.id ? (
                            ...
                            <>
                              ...
                              Vote for {token1?.name || formatAddress(battle.token1)}
                            </>
                          )}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>Click to vote for {token1?.name || formatAddress(battle.token1)}</p>
                        <p className="text-xs text-muted-foreground">Must hold tokens to vote</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                      ...
                      <TooltipTrigger asChild>
                        ...
                        >
                          {isVoting === battle.id ? (
                            ...
                            <>
                              <TrendingUp className="mr-2 h-4 w-4" />
                              Vote for {token2?.name || formatAddress(battle.token2)}
                            </>
                          )}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>Click to vote for {token2?.name || formatAddress(battle.token2)}</p>
                        <p className="text-xs text-muted-foreground">Must hold tokens to vote</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
          ...
          {battle.settled && battle.winner && (
            <div className="flex items-center justify-center gap-2 text-primary">
              ...
              <span className="font-medium">
                Winner: {tokens?.find(t => t.token === battle.winner)?.name || 
